...
with tab_trends:
    if not df.empty:
        ...
        fig_w = update_black(go.Figure(go.Scatter(x=df['회차'], y=df['본훈련파워']/85, mode='lines+markers', line=dict(color='#FF4D00', width=3))))
        ...
        def get_ef(r):
            hrs = [int(x) for x in str(r['전체심박데이터']).split(',') if x.strip()][3:-1]
