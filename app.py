...
with tab_entry:
    ...
    with st.form(key="modern_entry_form"):
        ...
        # 본 훈련 시간 슬라이더 (이 값에 따라 입력칸 개수 결정)
        ...
        st.divider()
        st.write(f"📊 **심박수 입력 ({f_duration + 15}분 코스)**")
        ...
        if st.form_submit_button("🚀 SAVE TRAINING RECORD", use_container_width=True):
            # 본훈련 심박 추출 (10분 지점 index 2 ~ 본훈련 종료 지점)
