...
# --- [Gemini API 설정: 사용자님이 요청한 자동 모델 매칭 시스템] ---
gemini_ready = False
if gemini_installed:
    ...
    if api_key:
        try:
            ...
            # [핵심] 사용 가능한 모델 리스트를 훑어 generateContent를 지원하는 모델 자동 선택
            models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
            
            # 사용자님 키에서 권한이 있는 모델 중 우선순위에 따라 매칭
...
with tab_entry:
    ...
    for i in range(total_pts):
        with h_cols[i % 4]:
            ...
            hr_val = st.number_input(f"{i*5}m HR", value=def_hr, key=f"hr_final_v2_{i}", step=1)
...
# --- [TAB 2: 분석 (70분 수직 낙하 + 자동 매칭 Gemini + 로딩 스피너)] ---
with tab_analysis:
    if s_data is not None:
        ...
        # [수직 낙하 교정] t < 10 + current_dur 조건을 통해 70분 정시 하강 보장
        ...
        if gemini_ready:
            ...
            if pr := st.chat_input("훈련 데이터에 대해 물어보세요..."):
                st.session_state.messages.append({"role": "user", "content": pr})
                with chat_box:
                    with st.chat_message("user"): st.markdown(pr)
                
                # 로딩 스피너 (모래시계 효과)
                with st.spinner("🤖 Gemini 코치가 답변을 준비 중입니다..."):
                    try:
                        ...
                        st.error(f"응답 실패: {e}. (매칭된 모델: {ai_model.model_name})")
        else:
