...
st.set_page_config(page_title="Hyper-Aggressive Coach v9.994", layout="wide")
...
# 4. Sidebar
with st.sidebar:
    st.markdown("<h2 style='color:#FF4D00; letter-spacing:0.1em;'>DYNAMIC COACH v9.994</h2>", unsafe_allow_html=True)
    if not df.empty:
...
with tab_entry:
    ...
    if w_type == "ZONE 2":
        f_main_dur = c3.slider("Main Set Duration (min)", 15, 180, 75, step=15)
        ...
        # 웜업 10분 + 본세션 + 쿨다운 5분
        f_total_duration = 10 + f_main_dur + 5 
        ...
        r1 = st.columns(5)
        f_sst_work, f_sst_rec, f_sst_sets, f_sst_work_t, f_sst_rec_t = r1[0].number_input("Power (W)", 185), r1[1].number_input("Rec (W)", 90), r1[2].number_input("Sets", 2), r1[3].number_input("Work(m)", 10), r1[4].number_input("Rec(m)", 5)
        r2 = st.columns(5)
        f_sst_w_s, f_sst_w_e, f_sst_c_s, f_sst_c_e = r2[0].number_input("WU_S (W)", 95), r2[1].number_input("WU_E (W)", 110), r2[2].number_input("CD_S (W)", 100), r2[3].number_input("CD_E (W)", 80)
        f_total_duration = 10 + (f_sst_sets * (f_sst_work_t + f_sst_rec_t)) + 15 
        f_mp = f_sst_work
        f_sst_p_data = f"SST,{f_sst_w_s},{f_sst_w_e},{f_sst_work},{f_sst_rec},{f_sst_c_s},{f_sst_c_e},{f_sst_sets},{f_sst_work_t},{f_sst_rec_t}"

    st.markdown(f'<p class="section-title">Heart Rate Data Entry (Total {f_total_duration} min)</p>', unsafe_allow_html=True)
    total_pts = (f_total_duration // 5) + 1
    hr_inputs = []
    for r_idx in range((total_pts + 3) // 4):
        ...
        for c_idx in range(4):
            ...
            if idx < total_pts:
                with cols[c_idx]:
                    hv = st.number_input(f"T + {idx*5}m", value=130, key=f"hr_v9994_{idx}")
                    hr_inputs.append(str(int(hv)))
    
    if st.button("SUBMIT WORKOUT DATA"):
        m_hrs = [int(x) for x in hr_inputs[2:-1]] # 웜업 제외 본세션 추출
        mid = len(m_hrs)//2
        f_ef = f_mp / np.mean(m_hrs[:mid]) if mid>0 else 0
        s_ef = f_mp / np.mean(m_hrs[mid:]) if mid>0 else 0
        new_row = {
            "날짜": f_date.strftime("%Y-%m-%d"), "회차": int(f_session), "훈련타입": w_type,
            "본훈련파워": int(f_mp), "본훈련시간": int(f_total_duration - 15), # 순수 훈련시간 저장
            "디커플링(%)": round(((f_ef-s_ef)/f_ef)*100,2) if f_ef>0 else 0,
            "전체심박데이터": ", ".join(hr_inputs), "파워데이터상세": f_sst_p_data
        }
...
with tab_analysis:
    if s_data is not None:
        ...
        if not z2_df.empty:
            ...
            if p_dec < 6.5: n_pres, coach_msg = f"{p_p+5}W / {max(p_dur, 75)}m", f"디커플링 {p_dec}%! 바닥 공사 완료. 즉시 상향합니다."
            else: n_pres, coach_msg = f"{p_p}W / {p_dur}m", "현재 강도 유지."
        else: n_pres, coach_msg = "145W / 75m", "데이터 수집 시작."

        st.markdown('<p class="section-title">Performance Briefing</p>', unsafe_allow_html=True)
        ca, cb = st.columns(2)
        with ca: st.markdown(f'<div class="briefing-card"><span class="prescription-badge">{s_data["훈련타입"]} RESULT</span><p style="font-size:1.5rem; font-weight:600; margin:0;">{int(s_data["본훈련파워"])}W / {int(s_data["본훈련시간"])}m</p><p style="color:#A1A1AA;">Decoupling: <b>{s_data["디커플링(%)"]}%</b></p></div>', unsafe_allow_html=True)
        ...
        else: p_y = [int(s_data['본훈련파워']) if 10 <= t <= 10+int(s_data['본훈련시간']) else 97 for t in time_x]
        ...
        fig_corr.update_layout(template="plotly_dark", plot_bgcolor='black', paper_bgcolor='black', xaxis=dict(gridcolor='#27272a'), yaxis=dict(gridcolor='#27272a'))
