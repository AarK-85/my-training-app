...
with tab_entry:
    ...
    if w_type == "ZONE 2":
        ...
        r1 = st.columns(5)
        f_sst_work, f_sst_rec, f_sst_sets, f_sst_work_t, f_sst_rec_t = r1[0].number_input("Steady-State Power (W)", 180), r1[1].number_input("Recovery Power (W)", 90), r1[2].number_input("Steady-State Sets", 2), r1[3].number_input("SS Time (min/set)", 10), r1[4].number_input("Recovery Time (min/set)", 5)
        r2 = st.columns(5)
...
with tab_analysis:
    if s_data is not None:
        ...
        ca, cb = st.columns(2)
        with ca:
            st.markdown(f"""<div class="briefing-card"><span class="prescription-badge">{c_type} ANALYSIS</span>
